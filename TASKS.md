...
### relase-firebase
    A [develop → merged to release]: Crashlytics SDK + dSYM upload — `FirebaseCrashlytics` SPM product added to the iPhone app target only (no Analytics, no Watch/widget); an "Upload Crashlytics dSYMs" run-script build phase (dSYM + Info.plist input files) so release traces symbolicate instead of showing raw addresses
    B [develop → merged to release]: Guarded init — `FirebaseApp.configure()` as the first line of `didFinishLaunchingWithOptions`, wrapped in a `Bundle.main.url(forResource: "GoogleService-Info")` check so it no-ops without the plist; registers the crash handler before the launch `Task`s so startup crashes are caught
...
### IOS-00122 Mobility category — rehab/mobility plans stop masquerading as strength
    - problem: rehab plans (stretching, isometrics, activation) had no domain representation in any of the three enums — the LLM scan forced them into "strength", Summary demanded a reps×kg set table for 30-second stretches, and analytics dumped every move into "Unknown Exercise"/Mixed
    A: Catalog v3 — 21 mobility/rehab cases (rehab-plan set: cat-cow, dead bug, bird dog, child's pose…; thoracic set: open book, thread the needle…; canonical stretches: pigeon, couch, 90/90, WGS, pass-throughs, downward dog) with EN+PL aliases, new `MovementCategory.mobility` (mint), catalogVersion 4 (v3 = new cases; v4 = paraphrase aliases harvested from the first on-device scan — the device had already consumed the v3 re-match) + golden tests; versioned re-match repairs history
    B: `ExerciseWorkoutType.mobility` — MOBILITY badge, confirmation-only logging in Summary (no kg table, ✓ pill once entered), score = `.completed`
    C: Scan classification — `SectionType.mobility` + Claude prompt rules (set-scheme notation alone ≠ strength; cooldown only for closing notes; heat packs are not exercises), mapper keeps mobility exercises as a full workout section (warmup/cooldown mapping would drop them as free text); workout-type if-chain refactored to exhaustive switch (a future SectionType case = compile error, not a silent AMRAP); mobility ignores the `rounds` format hint
    E: Set-scheme presentation — plan preview (WorkoutDetailContent, shared by scan preview + plan detail) renders plannedSets as explicit "Seria N" rows with the cue note below (scheme line stripped from `info`); Summary mobility cards log per-set rows via SetTable with a new `showsWeight: false` variant (kg column dropped, reps-only), entered ladder shows ✓ for set-based rows; strength paths untouched (default parameter)
    F: Exercise editor — "Sets" becomes a Target TYPE (Reps | Calories | … | Sets), not a separate section: picking Sets swaps the value row for a set-count stepper + per-set reps steppers (single source of truth — no more Reps living in two places); UI-only `ExerciseTargetType.sets` routes to `plannedSets` (data model unchanged), value carries over on reps ⇄ sets switches, per-set scanned weight suggestion survives reps edits; the hidden save-time `PlannedSet.parse` of the Notes text is removed — Notes is a plain note now
    - known limitation: `WorkoutSessionResult` doesn't persist wodType — historical mobility results derive as For Time via `derivedWodType`; proper fix = persist the type in the result (backlog)
    - decision: "Prone Weight Transfer" / "Oblique Isometric Hold" stay `.unknown` until the Unrecognized-names radar confirms recurrence (candidate for v4)
...
### IOS-00127 PR Board — dark redesign, all score types, Rx/Scaled split (closes milestone M-1)
    ...
    - S-03 all-score-types-controls DONE: one `parsedScore` validator per movement type; time via `TimePickerField` in its own Result section (palette blended into the Form surface), AMRAP via `DNFFields` stepper tiles, reps via numeric row; `supportsEntryForm` guard removed — every catalog movement accepts entries; review follow-up F3: chart filters entries to the movement's score type (D4 consistency)
...
### IOS-00128 PR suggestions from Summary — the catalog bridge goes live
    - branch: `dev/IOS-00128/IOS-00128`; also closes milestone M-1 in roadmap.md (S-03/S-04 flipped to done)
    - after a workout, the Results section compares each strength exercise's heaviest entered set against the PR Board (via the previously dormant `PRMovement.exerciseType` bridge) and shows a "New record?" card when the set beats the current best — or when the movement has no entries yet (board seeding); field-tested on device
    - tapping opens the PR entry editor prefilled: weight from the set, date = workout day (`HKWorkout.startDate`, fallback: session date), context = In WOD (the nil-context work pays off); user confirms Rx/equipment consciously — no silent auto-insert
    - `PRSuggestionBuilder` (new, pure): strength-only by design (benchmarks/timed cardio are tested separately and entered by hand — auto-detection sketch parked in roadmap); ambiguous reverse bridges (`.deadlift` → 2 movements) resolved by catalog order; a mismatched-type best (D4 fallback) yields no suggestion rather than comparing kg to seconds
    - `WorkoutResultsFeature` grew a one-shot `prEntries` snapshot (`PREntryClient.fetchAll`, new) instead of `@FetchAll` — the State must stay Equatable for the read-only ActivityDetails embed, which is double-guarded to never suggest or read; snapshot refreshes when the editor sheet closes, so a beaten suggestion disappears by itself
...
### IOS-00133 Watch workout recovery — auto-save after crash / battery death
    - branch: `dev/IOS-00133/IOS-00133`
    - gap found on a real workout: Watch battery died mid-watchPrimary session → `finishWorkout()` never ran → workout lost, even though HealthKit had all samples persisted outside the process; recovery existed but hid behind a launch alert ("End now" / destructive "Discard") the user may never see (fullScreenCover covers it, background relaunch renders no scene)
    - recovery now auto-finalizes without asking: single-flight `recoverStuckSession` (recover → normal `end()` flow → workout saved), alert + discard path removed — an unwanted save is deletable in Health, a discarded workout is gone forever
    - `handleActiveWorkoutRecovery()` in `WatchAppDelegate` — the system crash-relaunch hook recovers directly (via `@Dependency` client), covering background relaunches where `onAppear` never fires
    - launch race fixed: recovery completes BEFORE subscribing to `WorkoutConfigurationStream` (buffered, nothing lost), so a fresh start can no longer fight the recovered session over the manager's session/builder
    - accepted gaps: iPhone in-memory extras (effort points, plan link, class recap) not recoverable — future ticket; Apple does not document recovery after a full power-off boot → manual device test (drain battery mid-workout, recharge, launch, check Health)
    - review round (3 subagents; existing flows traced clean): bounded `stopActivityAndWait` (10 s timeout + continuation resumed on `.ended` and `didFailWithError` — a zombie session may never report `.stopped`, and the sequenced launch effect would have blocked all future starts), `start()` gated behind the same single-flight recovery (closes the WC `.workoutStarted` fallback race on all 3 entry points), transient HK failures no longer cached as "nothing to recover" (retryable results clear the cache), recovery snapshot delivered once (no duplicate log transfer per re-appear); leftover: 4 orphaned alert keys in Watch `Localizable.xcstrings` to delete via Xcode UI
    - low-battery warning at ≤5% (D): new `WatchDeviceClient` (polled `batteryLevel` — watchOS has no low-battery callback — plus warning haptic), once-a-minute poll during the session, one-shot full-screen overlay "End workout / OK" — ending now is the only guaranteed save, since post-power-off recovery is undocumented; no banner, no auto-end by design; system presentations over the paged TabView proved broken on watchOS 26 (alert → UIKit assertion trap, sheet → rendered but dead to touch on device), so the warning joined the proven saving/summary overlay chain (PL translations of the 4 new keys pending in Xcode)
    - review round 2 (post-commit): `sessionStoppedContinuation` double-resume race fixed with a dedicated lock (timeout task vs delegate callbacks — CheckedContinuation traps on double resume); 5% threshold compared as rounded integer percent (`Double(Float(0.05)) > 0.05` silently made the guard behave as ≤4% — found on a real device test); late battery poll can no longer sheet over the "Saving…"/summary screen
