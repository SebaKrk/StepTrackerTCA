...
### IOS-00133 Watch workout recovery — auto-save after crash / battery death
    ...
    - low-battery warning at ≤5% (D): new `WatchDeviceClient` (polled `batteryLevel` — watchOS has no low-battery callback — plus warning haptic), once-a-minute poll during the session, one-shot full-screen sheet "End workout / OK" — ending now is the only guaranteed save, since post-power-off recovery is undocumented; no banner, no auto-end by design (a system alert on the paged TabView tripped a deterministic UIKit assertion in preview, so the warning is a plain sheet; PL translations of the 4 new keys pending in Xcode)
