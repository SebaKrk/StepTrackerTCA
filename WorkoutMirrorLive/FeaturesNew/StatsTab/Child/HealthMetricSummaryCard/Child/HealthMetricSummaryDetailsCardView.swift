            ...
            metricCard(title: String(localized: "Value")) {
                HStack(alignment: .firstTextBaseline, spacing: 4) {
                    Text(formatValue(store.initialData.currentValue, unit: store.initialData.unit))
                        ...
                        .contentTransition(.numericText(value: store.initialData.currentValue))
                        .animation(.snappy(duration: 0.3), value: store.initialData.currentValue)
            ...
            metricCard(title: String(localized: "Score")) {
                HStack(alignment: .firstTextBaseline, spacing: 4) {
                    Text("\(store.initialData.score)")
                        ...
                        .fontWeight(.semibold)
                        .foregroundColor(.primary)
                        .contentTransition(.numericText(value: Double(store.initialData.score)))
                        .animation(.snappy(duration: 0.3), value: store.initialData.score)
                    Text("points")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
