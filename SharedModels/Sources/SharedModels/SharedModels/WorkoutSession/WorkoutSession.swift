...
/// ## Contracts
///
/// - `prepare()` MUST be called before `start(at:)`. Gives the HR sensor (Watch wrist
///   or BLE strap) time to warmup. Skipping causes iOS 26 mirroring disconnects
///   (Apple Developer Forums #804276 / FB20723311).
///
/// - `end()` implementation MUST call the underlying `HKWorkoutSession.end()` even if
///   `builder.endCollection(at:)` throws, otherwise the session stays in zombie state
///   and blocks the next workout.
///
/// - `workout` stream MUST be backed by `HKAnchoredObjectQueryDescriptor.results(for:)`
///   (iOS 15.4+), push-based AsyncSequence — NOT polling. Single emit when the session
///   ends and the `HKWorkout` lands in the HealthKit store.
///
/// - `metrics`, `state`, `workout` MUST be implemented as **computed properties** that
///   return a fresh `AsyncStream` on each access. `AsyncStream` only supports one
///   iterator; a stored stream paired with TCA cancellation results in the next
///   `for await` receiving nothing.
